...
typedef struct VCR_s
{
	...
	long		(*Hook_RegSetValueEx)(void *hKey, tchar const *lpValueName, unsigned long Reserved, unsigned long dwType, uint8_t const *lpData, unsigned long cbData);
	long		(*Hook_RegQueryValueEx)(void *hKey, tchar const *lpValueName, unsigned long *lpReserved, unsigned long *lpType, uint8_t*lpData, unsigned long *lpcbData);
	...
	unsigned long (*Hook_WaitForMultipleObjects)(uint32_t nHandles, const void **pHandles, int bWaitAll, uint32_t timeout );
