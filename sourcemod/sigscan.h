...
private:
    ...
    unsigned char sig_str[SIG_STRING_MAX];
...
public:
    ...
    /* Public Functions */
    CSigScan(void): sig_len(0), sig_addr(NULL) {}
    ~CSigScan(void) {}
 
    static bool GetDllMemInfo(void);
    static bool GetDllMemInfo(const char* modulename);
    void Init(unsigned char *sig, const char *mask, size_t len);
    void Init(const char* sig, const char* mask, size_t len) {
		Init((unsigned char*)sig, mask, len);
    }
};
