...
template <typename T, typename U>
[[nodiscard]] InlineHook create_inline_off(T target, int offset, U destination, InlineHook::Flags flags = InlineHook::Default) {
    return create_inline(reinterpret_cast<uintptr_t>(target) + offset, destination, flags);
}
